...
| `session-signal-stream.ts` | **Phase 40 — Session Execution Intelligence (RADICAL INNOVATION)** (~230 lines). Observable AI decision stream that captures WHY each trade decision was made. `SessionSignalStream` class: 5 event types (SIGNAL_EVALUATED, TRADE_EXECUTED, TRADE_SKIPPED, EXIT_TRIGGERED, ERROR_OCCURRED), 7 skip reasons (NO_CHAMPION, HOLD_SIGNAL, LOW_CONFIDENCE, MAX_POSITIONS, ALREADY_POSITIONED, COOLDOWN_ACTIVE, DRY_RUN). Pub/sub for real-time UI updates, execution rate statistics, max 50 events ring buffer. Singleton via `getSignalStream()`. | 🔴 |
...
| `system-bootstrap.ts` | **Phase 36→38.1 — System Bootstrap Orchestrator** (~662 lines). Singleton 7-phase ignition sequence. **Phase 38.1 FIX**: ENV_CHECK calls server-side `/api/trading/testnet-probe` (not client-side `validateEnvironment()`). **RADICAL INNOVATION: Probe Result Cache Reuse** — `BootConfig.cachedProbeResult` field enables fast path (cache <60s: ~0ms) / slow path (API: ~2s). ENV_CHECK: 2.1s → 6ms. Event loop yielding, 400ms min display, enhanced demo messages. | 🔴 |
...
| `pipeline/page.tsx` | **Pipeline Dashboard**. 15 panels: Pipeline Flow (7-stage animated), Generation Fitness (area chart), 4-Gate Validation (animated gates), Strategy Roster (radar), Experience Replay (heatmap), **Gene Lineage Tree** (family tree), **Gene Survival Heatmap** (persistence grid), **Decision Explainer** (regime reasoning), **Overmind Intelligence Hub**, **Live Pulse Telemetry** (ADFI+CIRPN, LIVE mode), **Evolution Heartbeat** (convergence detector, LIVE mode), **Risk Shield** (Risk Fortress), **StressMatrixPanel** (dual-mode LIVE/DEMO, 5-axis radar, RRS gauge, ASC heatmap, STTA sparkline + vulnerability heatmap), **TestnetMissionControlPanel** (probe dashboard, session lifecycle), **TestnetSessionPanel** (Phase 40, session control + SEI signal stream). Live state machine + dual-mode (LIVE/DEMO) + island selector. ~5238 lines. | 🟡 |
| `globals.css` | Premium design system. CSS custom properties for dark glassmorphism theme + gradient accents, stagger animations, neural map styles, pipeline stages, archaeology panels, **holographic brain theme** (3D canvas, scanlines, hex grid, neuron wireframes, synapse animations, HUD, consciousness arc, heatmap multi-color), **ignition system** (boot phases, waterfall chart, result badges, elapsed timer, boot history), **session panel** (Phase 40: config form, phase progress bar, stat tiles, signal stream feed). ~4564 lines. | 🟡 |
...
| `TestnetSessionPanel.tsx` | **Phase 40 — Testnet Session Panel** (~500 lines). NEW: Live paper trading session control. Pre-session config form (pairs, timeframe, capital, duration, dry run), 5-phase progress bar (PROBE→SEED→EVOLVE→TRADE→REPORT), live stat tiles (Elapsed, Trades, Open, P&L), auto-trade active badge, Start/Stop with safety confirmation, post-session report (P&L, win rate, max DD, best/worst). **RADICAL INNOVATION**: SignalStreamDisplay (85L) — real-time AI decision feed from `SessionSignalStream`, execution stats header, reverse-chronological color-coded event list. Uses `useSessionStore`. | 🔴 |
